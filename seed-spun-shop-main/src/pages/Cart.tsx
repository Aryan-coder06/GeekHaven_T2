...
export default function Cart() {
  const {
    items,
    itemCount,                 // <-- NEW: sum of quantities
    removeItem,
    updateQuantity,
    clearCart,
    formattedSubtotal,
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-4">
          {items.map((item, idx) => {
            const unitPrice = Number(item.price) || 0; // guard against undefined
            return (
              <Card key={`${item.id}-${idx}`} className="marketplace-card">
                <CardContent className="p-6">
                  <div className="flex items-center space-x-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="w-8 text-center font-medium">{Number(item.quantity) || 0}</span>
            ...
            <CardContent className="p-6">
              ...
              <div className="mt-4 p-3 bg-muted/50 rounded-lg">
                <p className="text-xs text-muted-foreground">
                  💡 Platform fee is calculated by your seed (FRONT25).
                </p>
              </div>
            </CardContent>
          </Card>
